...
// number of cells);
// climate data, given by a vector (tdr) of length 'N ';
...
model {
  ...
  occ ~ binomial_logit(checklists, coeff_tdr*tdr+inter);
  // for(i in 1:N){
  //   occ[i] ~ binomial_logit(checklists[i], coeff_tdr*tdr[i]+inter);
  // }
